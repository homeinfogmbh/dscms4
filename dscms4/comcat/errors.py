...
ERRORS = {
    Address.DoesNotExist: lambda _: JSONMessage("No such address.", status=404),
    ...
    InvalidAddress: lambda _: JSONMessage("Invalid value for address.", status=400),
    MenuBaseChart.DoesNotExist: lambda _: JSONMessage(
        "No such menu base chart.", status=404
    ),
    NoConfigurationFound: lambda _: JSONMessage("No configuration found.", status=400),
    Tenement.DoesNotExist: lambda _: JSONMessage(
        "The requested tenement does not exist.", status=404
    ),
    User.DoesNotExist: lambda _: JSONMessage("No such user.", status=404),
    ...
    UserMenu.DoesNotExist: lambda _: JSONMessage("No such user menu.", status=404),
}
