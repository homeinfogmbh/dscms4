...
def list_offer_reports() -> JSON:
    ...
    return JSON(
        [
            report.to_json()
            for report in OfferReport.select()
            .join(User)
            .join(Tenement)
            .where(Tenement.customer == CUSTOMER.id)
        ]
    )
...
def list_topic_reports() -> JSON:
    ...
    return JSON(
        [
            report.to_json()
            for report in TopicReport.select()
            .join(User)
            .join(Tenement)
            .where(Tenement.customer == CUSTOMER.id)
        ]
...
def list_response_reports() -> JSON:
    ...
    return JSON(
        [
            report.to_json()
            for report in ResponseReport.select()
            .join(User)
            .join(Tenement)
            .where(Tenement.customer == CUSTOMER.id)
        ]
...
def list_user_event_reports() -> JSON:
    ...
    return JSON(
        [
            report.to_json()
            for report in UserEventReport.select()
            .join(User)
            .join(Tenement)
            .where(Tenement.customer == CUSTOMER.id)
        ]
...
def delete_offer_report(ident: int) -> JSONMessage:
    ...
    OfferReport.select().join(User).join(Tenement).where(
        (Tenement.customer == CUSTOMER.id) & (OfferReport.id == ident)
    ).get().delete_instance()
...
def delete_topic_report(ident: int) -> JSONMessage:
    ...
    TopicReport.select().join(User).join(Tenement).where(
        (Tenement.customer == CUSTOMER.id) & (TopicReport.id == ident)
    ).get().delete_instance()
...
def delete_response_report(ident: int) -> JSONMessage:
    ...
    ResponseReport.select().join(User).join(Tenement).where(
        (Tenement.customer == CUSTOMER.id) & (ResponseReport.id == ident)
    ).get().delete_instance()
...
def delete_user_event_report(ident: int) -> JSONMessage:
    ...
    UserEventReport.select().join(User).join(Tenement).where(
        (Tenement.customer == CUSTOMER.id) & (UserEventReport.id == ident)
    ).get().delete_instance()
