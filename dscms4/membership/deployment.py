...
def list_deployment_configurations() -> JSON:
    ...
    return JSON(
        [
            dc.to_json()
            for dc in DeploymentConfiguration.select()
            .join(Deployment)
            .where(Deployment.customer == CUSTOMER.id)
        ]
    )
...
def list_deployment_base_charts() -> JSON:
    ...
    return JSON(
        [
            dbc.to_json()
            for dbc in DeploymentBaseChart.select()
            .join(Deployment)
            .where(Deployment.customer == CUSTOMER.id)
        ]
    )
...
def list_deployment_menus() -> JSON:
    ...
    return JSON(
        [
            dm.to_json()
            for dm in DeploymentMenu.select()
            .join(Deployment)
            .where(Deployment.customer == CUSTOMER.id)
        ]
    )
...
ROUTES = [
    ("GET", "/membership/deployment/configuration", list_deployment_configurations),
