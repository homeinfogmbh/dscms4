...
def list_group_configurations() -> JSON:
    ...
    return JSON([
        gc.to_json() for gc in GroupConfiguration.select().join(Group).where(
...
def list_group_base_charts() -> JSON:
    ...
    return JSON([
        gbc.to_json() for gbc in GroupBaseChart.select().join(Group).where(
            Group.customer == CUSTOMER.id
        )
    ])
...
def list_group_menus() -> JSON:
    ...
    return JSON([
        gm.to_json() for gm in GroupMenu.select().join(Group).where(
            Group.customer == CUSTOMER.id
        )
    ])
