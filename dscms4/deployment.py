...
def list_() -> JSON:
    ...
    deployments = get_deployments(
        CUSTOMER.id, testing=settings.testing, content=(assoc := get_bool("assoc"))
    )

    if assoc:
        return JSON({deployment.id: deployment.to_json() for deployment in deployments})

    return JSON(
        [deployment.to_json(address=True, systems=True) for deployment in deployments]
    )
